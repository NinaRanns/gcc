...
test01()
{
  using O = std::pmr::optional<T>;
   using S = T::state_type;
   auto const make = [](S s = S::zero) { return T { s }; };
...

   {
     ...
     O o = make();
     T v = make(S::throwing_copy_assignment);
...

   {
     outcome_type outcome {};
     O o = make();
     T v = make(S::throwing_move_assignment);
