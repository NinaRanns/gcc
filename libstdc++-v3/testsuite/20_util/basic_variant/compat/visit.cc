...
test01()
{
  ...
  std::basic_variant<std::allocator<int>,X, X*, std::reference_wrapper<X>> vobj{X{1}};
  int res = std::visit(&X::n, vobj);
  VERIFY( res == 1 );

  std::basic_variant<std::allocator<int>,int, short> varg{2};
...
test02()
{
  ...
  std::basic_variant<std::allocator<int>,NoCopy, int> v{1};
  NoCopy f;
