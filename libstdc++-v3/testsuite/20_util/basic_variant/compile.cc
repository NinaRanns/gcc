...
struct TrivialAlloc{
   /// The allocator type
    ...
    template<typename U>
    operator TrivialAlloc<U>(){ return TrivialAlloc<U>{};}
};
...
void copy_ctor()
{
  ...
  static_assert(!is_trivially_copy_constructible_v<basic_variant<std::allocator<int>, int>>);
  static_assert(!is_trivially_copy_constructible_v<basic_variant<TrivialAlloc<int>, std::string>>);
  static_assert(!is_trivially_copy_constructible_v<basic_variant<std::allocator<int>, DeletedMoves>>);
...
#define TEST_TEMPLATE(DT, CC, MC, CA, MA, CC_VAL, MC_VAL, CA_VAL, MA_VAL) \
  { \
    ...
    static_assert(CC_VAL == is_trivially_copy_constructible_v<basic_variant<TrivialAlloc<int>, A>>); \
    static_assert(MC_VAL == is_trivially_move_constructible_v<basic_variant<TrivialAlloc<int>, A>>); \
    static_assert(CA_VAL == is_trivially_copy_assignable_v<basic_variant<TrivialAlloc<int>, A>>); \
    static_assert(MA_VAL == is_trivially_move_assignable_v<basic_variant<TrivialAlloc<int>, A>>); \
  }
