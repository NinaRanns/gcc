...
struct X {
  ...
  constexpr X( X&& other) : value(other.value  + move_value), alloc(other.alloc){};
  constexpr X(std::allocator_arg_t, const allocator_type& a,  X&& other):
          value(other.value + move_value ), alloc(a) {};
...
{
  {
    ...
    constexpr basic_variant<MyAlloc<X>, int, X> copy = std::move(obj);
    static_assert(get<int>(copy) == 0);
    static_assert(copy.get_allocator().id == 0);
  }
  {
    constexpr basic_variant<MyAlloc<X>, X> obj;
    constexpr basic_variant<MyAlloc<X>, X> copy = std::move(obj);
  ...
  }
  {
    ...
    constexpr basic_variant<std::allocator<X>, X> copy = std::move(obj);
  ...
  static_assert(is_move_constructible_v<basic_variant<MyAlloc<X>, int, string>>);
  static_assert(is_move_constructible_v<basic_variant<MyAlloc<X>, string, string>>);
  static_assert(!is_move_constructible_v<basic_variant<MyAlloc<X>, string, AllDeleted>>);
  static_assert(is_move_constructible_v<basic_variant<MyAlloc<X>, DeletedMoves>>);


  static_assert(is_trivially_move_constructible_v<basic_variant<MyAlloc<X>, int>>);
  static_assert(!is_trivially_move_constructible_v<basic_variant<MyAlloc<X>, X>>);
  static_assert(!is_trivially_move_constructible_v<basic_variant<std::allocator<X>, X, int>>);
  static_assert(!is_trivially_move_constructible_v<basic_variant<std::allocator<int>, char, int>>);
...
int main()
{
...

  {
    ...
    MyAllocVar copy1(std::move(obj));
    VERIFY(get<X>(copy1).value == 0 + move_value);
...

  {
    ...
    MyAllocVar copy1(std::allocator_arg, MyAlloc<int>{6},std::move(obj));
    VERIFY(get<X>(copy1).value == 0 + move_value);
...

  {
    ...
    MyAllocVar copy1(std::allocator_arg, MyAlloc<X>{6},std::move(obj));
    VERIFY(get<X>(copy1).value == 0 + move_value);
    VERIFY(get<X>(copy1).alloc.id == 6);
    VERIFY(copy1.get_allocator().id == 6);
  }
  ...
  {
    std::basic_variant<MyAlloc<X>, X, int> obj{std::allocator_arg, MyAlloc<int>{5}};
...

  {
    ...
    PmrVar copy1(std::allocator_arg, pmrAlloc{}, std::move(obj));
    VERIFY(get<X>(copy1).value == 0 + move_value);
    VERIFY(get<X>(copy1).alloc.id == -1);
  }
